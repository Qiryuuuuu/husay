...
app.post("/rfid-answer", async (req, res) => {
  try {
    ...
    const newRFIDAnswer = new RFIDAnswer({ category, answer, timestamp: new Date() });
    await newRFIDAnswer.save();

    res.status(201).json({ success: true, message: "New RFID Answer stored", data: newRFIDAnswer });
  } catch (error) {
...
app.get("/latest-rfid-answer", async (req, res) => {
  try {
    ...
    if (!latestAnswer) {
      return res.status(404).json({ success: false, message: "No RFID answers found." });
    }
...
app.get("/latest-rfid-result", async (req, res) => {
  try {
    ...
    if (!latestResult) {
      return res.status(404).json({ success: false, message: "No RFID results found." });
    }
...
app.post("/data", async (req, res) => {
  try {
    ...
    if (!latestResult) {
      latestResult = await RFIDResult.create({ result: "pending", timestamp: new Date() });
    }
